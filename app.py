...
@st.cache_data
def load_data():
    #df = pd.read_csv('C:/Users/dhima/OneDrive/Desktop/gamerec-main(1)/gamerec-main/video_game_data(1).csv') ####changes 
                ...
                with col1:
                    ...
                    for idx, row in recommendations.iterrows():
                        with st.container():
                            ...
                            with col_img:
                                st.image(get_game_image_url(row['title']), use_container_width=True)
                            with col_info:
            ...
            if st.button(f"Get Similar Games to {row['title'].title()}", key=f"popular_{idx}"):
                ...
                if success:
                    ...
                    for rec_idx, rec_row in recommendations.iloc[1:].iterrows():
                        ...
                        with col_rec_img:
                            st.image(get_game_image_url(rec_row['title']), use_container_width=True)
                        with col_rec_info:
